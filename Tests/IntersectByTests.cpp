...
struct IntersectByEquatable final
{
    auto EqualTo(const IntersectByEquatable other) const -> bool
    {
        ++equalityCount;
    ...
    auto operator==(const IntersectByEquatable right) const -> bool
    {
...
uint32_t IntersectByEquatable::equalityCount = 0U;

TEST(IntersectBy, ExecutionIsDeferred_DefaultComparer)
{
    // Arrange.
    IntersectByEquatable::equalityCount = 0U;

    const IntersectByEquatable source[]{ 'a', 'b', 'c', 'd', 'e' };
    const IntersectByEquatable intersect[]{ 'b', 'd', 'e', 'f', 'g' };

    // Act.
    const auto actual = From(source).IntersectBy(From(intersect), [](const IntersectByEquatable value){ return value; });

    // Assert.
    EXPECT_EQ(IntersectByEquatable::equalityCount, 0U);
}
...
TEST(IntersectBy, ExecutionIsDeferred_CustomComparer)
{
    ...
    // Act.
    const auto actual = From(source).IntersectBy(From(intersect), [](const IntersectByEquatable value){ return value; }, [](const IntersectByEquatable value1, const IntersectByEquatable value2){ return value1.EqualTo(value2); });
